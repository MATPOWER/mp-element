classdef mp_math_opf_acpi_legacy < mp_math_opf_acpi & mm_shared_opf_legacy
%MP_MATH_OPF_ACPI_LEGACY  MATPOWER mathematical model for AC optimal power flow (OPF) problem.
%   ?
%
%   MP_MATH_OPF_ACPI_LEGACY ... power flow ...
%
%   Properties
%       ? - ?
%
...
%     end
    ...
    methods
        %% constructor
        function obj = mp_math_opf_acpi_legacy()
            obj@mp_math_opf_acpi();
        ...
        function obj = add_named_set(obj, varargin)
            ...
            add_named_set@mp_math_opf_acpi(obj, varargin{:});
        ...
        function obj = init_set_types(obj)
            init_set_types@mp_math_opf_acpi(obj);
            obj.init_set_types_legacy();
        end

        function obj = build(obj, nm, dm, mpopt)
            obj.mpc = dm.source;
            build@mp_math_opf_acpi(obj, nm, dm, mpopt);
            obj.build_legacy(nm, dm, mpopt);
        end

        function obj = add_vars(obj, nm, dm, mpopt)
            add_vars@mp_math_opf_acpi(obj, nm, dm, mpopt);  %% call parent
        ...
        function add_system_costs(obj, nm, dm, mpopt)
            add_system_costs@mp_math_opf_acpi(obj, nm, dm, mpopt);  %% call parent
