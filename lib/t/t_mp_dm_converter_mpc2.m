...
t_is(length(dmc.element_classes), 10, 12, [t '# of element_classes']);
t_ok(isempty(dmc.elements), [t 'elements empty']);

t = 'dmc.build() : ';
dmc.build();
t_is(length(dmc.elements), 10, 12, [t '# of elements']);
...
t_is(length(dm.element_classes), 10, 12, [t '# of element_classes']);
