classdef mme_branch_opf_ac < mme_branch
            ...
            if nl2
                ...
                if lim_type == 'S'
                    fcn_flow = @(x)port_apparent_power_lim_fcn(nme, ...
                        mm.opf_convert_x(x, nm, mm.aux_data), nm, idx, ...
                        [flow_max; flow_max] .^ 2);
                    hess_flow = @(x, lam)port_apparent_power_lim_hess(nme, ...
                        mm.opf_convert_x(x, nm, mm.aux_data), lam, nm, idx);
                elseif lim_type == 'P'
                    fcn_flow = @(x)port_active_power_lim_fcn(nme, ...
                        mm.opf_convert_x(x, nm, mm.aux_data), nm, idx, [flow_max; flow_max]);
                ...
                elseif lim_type == 'I'
                    ...
                    error('mme_branch_opf_ac/add_constraints: MPOPT.opf.flow_lim = ''%s'' not yet implemented.', mpopt.opf.flow_lim);
                end
