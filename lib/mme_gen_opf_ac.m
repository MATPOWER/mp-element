    ...
    methods
        ...
        function TorF = has_pq_cap(obj, gen, upper_lower)
            ...
            if isempty(k)
                ...
                if any( gen.pc1(k) >= gen.pc2(k) )
                    error('mme_gen_opf_ac/has_pq_cap: must have pc1 < pc2');
                end
                if any( gen.qc2_ub(k) <= gen.qc2_lb(k) & gen.qc1_ub(k) <= gen.qc1_lb(k) )
                    error('mme_gen_opf_ac/has_pq_cap: capability curve defines an empty set');
                end
        ...
        function [A, l, u] = disp_load_constant_pf_constraint(obj, dm);
            ...
            if any( qg_lb(ivl) ~= 0 & qg_ub(ivl) ~= 0 )
                ...
                error('mme_gen_opf_ac/disp_load_constant_pf_constraint: Either qg_lb or qg_ub must be equal to zero for each dispatchable load.\n%s', s);
            end
            ...
            if any( abs( qg(ivl) - pg(ivl) .* Qlim ./ pg_lb(ivl) ) > 1e-6 )
                ...
                error('mme_gen_opf_ac/disp_load_constant_pf_constraint: %s\n         %s\n         %s\n         %s\n%s', ...
