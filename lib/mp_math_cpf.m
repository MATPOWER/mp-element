classdef (Abstract) mp_math_cpf < mp_math_pf
...
%     end
    ...
    methods
        ...
        function y = plot_yfcn(obj, nm, dm, ad, v_, bus_num)
            ...
            if ~isempty(k)
                error('mp_math_cpf/plot_yfcn: %d is not a valid bus number for CPF voltage plot', bus_idx(k));
            end
        ...
        function [nx, cx, s] = callback_qlim(obj, k, nx, cx, px, s, opt, nm, dm, mpopt)
            ...
            if ~isempty(ev)
                ad = obj.aux_data;
                if ad.nref ~= 1
                    error('mp_math_cpf/callback_qlim: ''cpf.enforce_qlims'' option only valid for systems with exactly one REF bus');
                end
        ...
        function [nx, cx, s] = callback_plim(obj, k, nx, cx, px, s, opt, nm, dm, mpopt)
            ...
            if ~isempty(ev)
                ad = obj.aux_data;
                if ad.nref ~= 1
                    error('mp_math_cpf/callback_plim: ''cpf.enforce_plims'' option only valid for systems with exactly one REF bus');
                end
