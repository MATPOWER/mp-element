function obj = t_acc_nln_port_inj_current(quiet)
%T_ACC_NLN_PORT_INJ_CURRENT  Tests of port_inj_current() derivatives wrt cartesian V.
...
ac = accs_nln_test_aggregate().create_model(mpc);
C = ac.C;
D = ac.D;
np = ac.np;
nv = ac.nv/2;
...
%  x0 = [va([pv; pq]); vm(pq)];
% pf_fcn = @(x)power_flow_equations(ac, x, va, vm, z_, ref, pv, pq);
